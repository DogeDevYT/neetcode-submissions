...
lets try to make it O(n) by utilizing a hashmap to store values
"""
class Solution:
    def __init__(self):
        self.heights = {} #use diciontary to store values and convert to
        #DP based solution
    ...
    def node_height(self, root: Optional[TreeNode]) -> int:
        ...
        if root in self.heights: return self.heights[root]

        #store return value to put in map
        self.heights[root] = 1 + max(self.node_height(root.left), self.node_height(root.right))
